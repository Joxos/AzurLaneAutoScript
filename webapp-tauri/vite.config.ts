...
// Dev-only API target: the python backend (started via `alas run web`, or a
// standalone instance). The SPA uses relative URLs, so without a proxy every
// /api-style call would hit the vite dev server itself, fail, and the UI
// would render raw i18n keys ("Gui.xxx").
